...
if Xiang:
    ...
    st.write(
        "However, with Streamlit, the role of developers is greatly reduced, data scientists could build the apps and iterate models by their own within a very short time. ")
    st.write("")
...
if Stian:
    ...
    st.pyplot(fig)
    st.text(
        "We still don't see some of the larger countries represented in the\nsegment of teams that won 40+ total medals")
    ...
    st.pyplot(fig)
    st.text(
        "This shows that India competed in the least amount of sports compared to the winningest\nand other populous countries")
